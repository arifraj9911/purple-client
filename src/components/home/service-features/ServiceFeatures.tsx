        ...
        <div className='grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4'>
          {features.map((feature) => (
            <Link
              key={feature.id}
              href={feature.link}
              className='group flex flex-col items-center rounded-lg border border-gray-300 bg-gray-50 p-4 md:pt-4 text-center transition-all duration-300 hover:-translate-y-1 hover:shadow-lg '
            >
              {/* Icon */}
              ...
              {/* Description */}
              <p className='mt-1.5 text-xs leading-tight text-gray-400 sm:mt-2 sm:text-sm'>
                {feature.description}
              </p>
            </Link>
          ))}
