...
export default function RelatedProducts({
  ...
  // Same category first, fall back to the rest of the catalogue
  const related = products.filter(
    (p) => p.id !== currentId && p.categoryId === categoryId,
  );
  const list = (
    related.length > 0 ? related : products.filter((p) => p.id !== currentId)
  ).slice(0, 8);
      ...
      <div className='container mx-auto px-4 md:px-6 lg:px-8'>
        ...
        <div className='mb-4 md:mb-8 flex items-end justify-between'>
          <div>
            ...
            <h2 className='mt-1 font-heading text-2xl font-bold text-gray-900 sm:text-3xl'>
              Related Products
            </h2>
          </div>
        ...
        <div className='overflow-hidden' ref={emblaRef}>
          <div className='flex items-stretch gap-3 sm:gap-5 md:gap-6'>
            {list.map((product) => (
              <div
                key={product.id}
                className='w-[62vw] min-w-0 shrink-0 sm:w-[45vw] md:w-[calc(33.333%-16px)] lg:w-[calc(25%-18px)]'
              >
